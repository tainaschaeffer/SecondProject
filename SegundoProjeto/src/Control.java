...
public class Control 
{
    ...
    public static void main(String[] args) throws Exception 
    {
        ...
        Map<String, List<Review>> avaliacoes = lines.stream()
                                                     .map(Control::convertToReview)
                                                     .collect(groupingBy(Review::getReleaseYear, TreeMap::new, toList()));

        controleBase(avaliacoes);
        exibirAnoLancamentoMaiorNumeroJogosAcao(avaliacoes);
    }
    ...
    private static void exibirAnoLancamentoMaiorNumeroJogosAcao(Map<String, List<Review>> avaliacoes) 
    {
        long biggestActionGamesAmount = 0;
        String yearBiggestActionGamesAmount = null;

        for (String year : avaliacoes.keySet()) 
        {
            long actionGamesAmount = avaliacoes.get(year).stream().filter(review -> review.getGenre().equals("Action")).map(Review::getGame).distinct().count();
    ...
    private static void controleBase(Map<String, List<Review>> avaliacoes) 
    {
        avaliacoes.forEach((year, reviews) -> 
        {
            double sumScore = 0;
            double mediocre = 0;
            ...
            List<String> piorParaMaior = ordenaDePiorParaMaior(reviews);
            ...
            System.out.println(String.format("Melhor jogo: %s", piorParaMaior.get(piorParaMaior.size() - 1)));
            System.out.println(String.format("Pior jogo: %s", piorParaMaior.get(0)));
            System.out.println("\n---------------------------------");
        });
    }

    private static List<String> ordenaDePiorParaMaior(List<Review> reviews) 
    {
